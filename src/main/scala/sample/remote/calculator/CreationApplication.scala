...
class CreationApplication extends Bootable {
  ...
  val localActorRef  = system.actorOf(Props[CreationActor].withDispatcher("durable-dispatcher"),           "creationActor")
  val remoteActorRef = system.actorOf(Props[AdvancedCalculatorActor].withDispatcher("durable-dispatcher"), "advancedCalculator")
