...
class LookupApplication extends Bootable {
  ...
  val actorRef = system.actorOf(Props[LookupActor].withDispatcher("durable-dispatcher"), "lookupActor")
  ...
  // Even when only 1 instance of CalculatorApplication is requested, I see 3 created
  val remoteActorRef = system.actorOf(Props[SimpleCalculatorActor].withDispatcher("durable-dispatcher").withRouter(RoundRobinRouter(1)))
  ...
  * be done reliably."*/
  ...
  }).withDispatcher("durable-dispatcher"), "RemoteClientLifeCycleListener")
